...
function ContactIcon({ name }: { name: "instagram" | "behance" | "linkedin" | "email" | "phone" }) {
  if (name === "email") {
    return (
    ...
    return (
      <span className="contact-icon" aria-hidden="true">
        <svg viewBox="0 0 24 24" className="contact-icon-svg">
          <path d="M9.1 6.9c.32-.32.8-.4 1.18-.2l1.5.78c.43.22.62.73.45 1.18l-.53 1.38c-.1.25-.04.54.16.73l1.91 1.91c.2.2.48.26.73.16l1.38-.53c.45-.17.96.02 1.18.45l.78 1.5c.2.38.12.86-.2 1.18l-.78.78c-.56.56-1.38.77-2.14.57-1.54-.42-3.09-1.42-4.64-2.97-1.55-1.55-2.55-3.1-2.97-4.64-.2-.76.01-1.58.57-2.14l.78-.78Z" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </span>
    );
  }
  ...
  if (name === "instagram") {
    return (
      <span className="contact-icon" aria-hidden="true">
        <svg viewBox="0 0 24 24" className="contact-icon-svg">
          <rect x="4.2" y="4.2" width="15.6" height="15.6" rx="4.4" ry="4.4" fill="none" stroke="currentColor" strokeWidth="1.6" />
  ...
  if (name === "behance") {
    return (
      <span className="contact-icon contact-icon--behance" aria-hidden="true">
        <svg viewBox="0 0 24 24" className="contact-icon-svg">
          <path d="M5 7.4h5.62c1.82 0 3 .96 3 2.48 0 1.04-.54 1.8-1.48 2.14 1.22.26 1.94 1.22 1.94 2.54 0 1.76-1.38 2.94-3.52 2.94H5V7.4Zm5.02 3.86c1 0 1.58-.42 1.58-1.14 0-.74-.58-1.12-1.58-1.12H7.08v2.26h2.94Zm.18 4.36c1.18 0 1.86-.48 1.86-1.36 0-.86-.68-1.3-1.86-1.3H7.08v2.66h3.12Z" fill="currentColor" />
          <path d="M15.38 8.05h3.72v1.04h-3.72z" fill="currentColor" />
          <path d="M19.42 14.8c-.3 1.6-1.46 2.58-3.08 2.58-2.1 0-3.4-1.38-3.4-3.58 0-2.2 1.32-3.62 3.34-3.62 2.02 0 3.28 1.38 3.28 3.54v.46h-4.72c.08 1 .64 1.62 1.54 1.62.66 0 1.16-.26 1.4-.76h1.64Zm-4.54-1.74h3.12c-.06-.94-.62-1.48-1.5-1.48-.86 0-1.44.54-1.62 1.48Z" fill="currentColor" />
        </svg>
      </span>
    );
  }
  ...
  return (
    <span className={`contact-icon ${name === "linkedin" ? "contact-icon--linkedin" : ""}`} aria-hidden="true">
      <svg viewBox="0 0 24 24" className="contact-icon-svg">
        <rect x="4.5" y="4.5" width="15" height="15" rx="1.8" fill="none" stroke="currentColor" strokeWidth="1.6" />
        <g transform="translate(-0.45 0)">
          <path d="M8 10h2v6H8z" fill="currentColor" />
          <circle cx="9" cy="7.8" r="1.1" fill="currentColor" />
          <path d="M12.2 10h1.9v.95c.4-.7 1.06-1.15 2.02-1.15 1.46 0 2.38.98 2.38 2.7V16h-2v-3.14c0-.98-.42-1.5-1.22-1.5-.84 0-1.34.6-1.34 1.58V16h-1.85v-6Z" fill="currentColor" />
        </g>
      </svg>
    </span>
  );
...
export default function ContactSection() {
  ...
  const [stickyPosition, setStickyPosition] = useState({ left: 50, top: 50, rotate: -10 });
  const [clickFeedback, setClickFeedback] = useState(false);
  const [stickyPhrase, setStickyPhrase] = useState(STICKY_PHRASES[0]);

  const confettiPieces = useMemo(
    () =>
      Array.from({ length: 22 }, (_, index) => {
        const fromLeft = index % 2 === 0;
        const horizontalMid = fromLeft ? -120 - (index % 5) * 32 : 120 + (index % 5) * 32;
        const verticalStart = -36 - (index % 5) * 12;
        const fallDrift = fromLeft ? -80 - (index % 5) * 28 : 80 + (index % 5) * 28;
        ...
        const shape = index % 3 === 0 ? "circle" : index % 3 === 1 ? "strip" : "square";
        const color =
          index % 4 === 0
            ? "#fbc740"
            : index % 4 === 1
  ...
  const moveSticky = useCallback((clientX: number, clientY: number) => {
